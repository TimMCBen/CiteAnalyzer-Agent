...
- `scripts/test_agent/stage7.py` 的真实 LLM 国家/地区解析 smoke 只在 GitHub CI 中强制运行；本地默认跳过。
- GitHub CI 需要仓库 Secrets 提供 `API_KEY` 和 `BASE_URL`，并由 workflow 固定设置 `MODEL=gpt-5.4`。
