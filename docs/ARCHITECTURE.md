...
如需查看更适合讲解和 onboarding 的阶段图、状态演化图与代码路径图，见 `docs/design-docs/citation-analysis-maps.md`。
...
- 标准化施引文献列表
- 经论文身份核验确认的 OpenAlex work
- `work.authorships.author.id` 和 authorship 机构信息
...
- `OpenAlex Works`
- `OpenAlex Authors by ID`
- `arXiv` metadata（辅助论文身份核验）

标注规则：

- Stage 4 不再按作者名搜索 OpenAlex / DBLP，也不再取名字搜索第一候选
- 只有论文匹配为高 / 中置信，且 OpenAlex work authorship 提供 `author.id` 时，才进入作者画像
- 论文不可信、缺少 selected work、缺少 authorship 或缺少 author id 时，直接跳过并写入日志和报告
...
- 该智能体优先保证作者身份可信度；宁可减少覆盖率，也不使用低置信 name search 污染重要学者和地图
...
- 该智能体可以在自己的任务域内选择调用 GROBID 和 LLM 分类能力，但不负责全局流程推进
...
- `reportlab`（独立 PDF 报告版式，不依赖浏览器打印 HTML）
- HTML / PDF 报告模板或导出能力
...
- 目标论文基础信息
- 引用数量与年份趋势
- 施引来源国家 / 地区地图（基于机构文本、规则与可选 LLM 辅助推断，只精确到国家/地区）
- 机构 Top N 分布
- 重量级学者分布与代表作者列表
- 引用情感饼图
- 主要观察结论与待人工关注项
...
- 当前实现同时保留机构 Top N 聚合和国家 / 地区级轻量标准化结果
- 国家 / 地区推断先按内置规则处理高置信机构；规则无法判断时可调用 LLM，并在报告 provenance 中记录 method / confidence / evidence
- 若一篇施引论文存在多个作者机构，第一版优先按首条机构文本统计
- 若机构信息缺失，则国家 / 地区分布记为 `Unknown` 并标记需复核
- HTML 图表层使用本地简化 GeoJSON 和 ECharts map 渲染“施引来源国家/地区地图”；`Unknown` 与未映射地区只在注释和 fallback 列表展示，不强行归并到某个国家

边界：

- 当前文档与产品规格对齐为 `HTML + JSON + PDF` 交付
...
- `packages/reporting` 只负责图表生成、HTML / JSON / PDF 报告导出和报告内国家/地区分布推断
...
- `packages/author_intel/`
  - 阶段 4 的 work-authorship 作者画像补全、OpenAlex author-id 查询和重量级学者规则
...
- `packages/sentiment/`
  ...
  - 当前默认方向是 `PDF-only` artifact，与 `PDF -> GROBID -> context` 主路径；不再执行直接 HTML / TeX / 摘要文本提取或 PDF 文本抽取路径
...
4. 总智能体按需调用 `学者识别智能体`
5. 总智能体按 PDF / GROBID 上下文可用性调用 `引用情感分析智能体`
6. 总智能体调用 `可视化报告智能体` 生成 HTML / JSON / PDF 报告
