...
| 测试 | B | 已有阶段 1 / 2 / 4 / 5 / 6 / 7 脚本验证，`stage7.py` 在 CI 中会真实调用 LLM 验证国家/地区解析，本地默认跳过 live smoke；`e2e_mvp.py` 已能跑 fixture-backed 全链路；但真实外部 API live 回归仍然偏少。 | 增加更多真实样本回归，尤其是 OpenAlex / DBLP live 路径和更多 PDF / HTML 全文样本。 |
