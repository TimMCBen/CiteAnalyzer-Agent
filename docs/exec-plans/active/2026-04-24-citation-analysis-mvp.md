...
- 已知约束：
  ...
  - `arXiv` 既作为输入兼容入口，也作为全文抓取阶段的优先公开源
  - HTML 为当前默认报告交付格式
  - 情感分析仅对成功提取上下文的记录执行
  - 重量级学者标注采用启发式规则
...
4. 全文抓取与文本解析阶段
   - 跑通 `arXiv-first` 全文获取与 PDF / HTML / LaTeX 解析
5. 引用情感分析智能体
   - 跑通上下文提取、分类与“无法判断”降级
6. 可视化报告智能体
   - 生成 HTML 报告与图表
...
- [ ] 明确 `FullTextDocument` 与 `TextSourceSelection` 最小字段结构
- [ ] 定义全文抓取阶段的状态输入输出接口
- [ ] 搭建 `packages/sentiment/fulltext.py` 的阶段边界
- [ ] 明确 `arXiv-first` 全文输入来源策略
- [ ] 实现 PDF / HTML / LaTeX 文本解析
- [ ] 定义抓取失败与解析失败的错误暴露方式
- [ ] 将全文抓取阶段接入总智能体状态图
- [ ] 用真实样本验证抓取与解析结果
...
- [ ] 添加 `scripts/test_agent/stage8.py` 阶段验证脚本
