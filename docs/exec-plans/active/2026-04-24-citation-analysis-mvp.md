...
完成 `CiteAnalyzer-Agent` 的 MVP 落地，构建一个以 `论文被引分析智能体` 为总控主体、以多个原始命名不变的子智能体协同工作的系统。首轮交付目标是：输入一篇目标论文后，能够完成施引文献抓取、学者识别、引用情感分析，并输出 HTML 可视化分析报告。
