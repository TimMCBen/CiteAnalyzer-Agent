...
- 配合：主导人
- 目标：
  - 为后续引用定位与情感分析准备稳定、可复用的全文产物
  - 完成 `PDF-first` 全文抓取、本地落盘和基础文本解析
- 交付物：
  - `FullTextDocument`
  - `raw artifact + parsed txt`
  - 可被阶段 6 消费的本地全文材料
- 依赖：
  - 阶段 2
...
- [x] 明确全文入口与 `PDF-first` artifact 契约
- [x] 搭建 `packages/sentiment/` 基础目录结构
- [x] 实现 `PDF-first` 全文抓取与本地落盘
- [x] 实现 PDF / HTML / LaTeX 解析
- [x] 添加 `scripts/test_agent/stage5.py` 阶段验证脚本
- [x] 完成本地夹具验证
- [ ] 增加更多真实 PDF 样本回归
- [x] 将阶段 5 产物正式接回总智能体状态图
- [ ] 更新 execution plan 的阶段进度
...
- 配合：主导人
- 目标：
  - 基于阶段 5 产物恢复目标论文引用位置并判断引用态度
  - 固化 `PDF -> GROBID -> context` 主路径和本地回退路径
- 交付物：
  - `CitationContext`
  - `SentimentSummary`
  - 可被总智能体调用的情感分析能力节点
...
- [x] 明确 `CitationContext` / `SentimentSummary` 最小字段结构
- [x] 定义阶段 6 工作流边界与回退策略
- [x] 实现 `LangGraph` 工作流骨架
- [x] 实现 GROBID `PDF -> TEI XML -> context` 主路径
- [x] 实现直接 TeX bibliography / cite-key 兼容路径
