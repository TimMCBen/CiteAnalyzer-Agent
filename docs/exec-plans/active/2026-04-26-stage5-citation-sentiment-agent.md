# 阶段 5 / 阶段 6 执行计划：全文抓取与引用情感工作流

## 目标

这份文件沿用历史文件名，但当前内容按最新共识同时维护两个相邻阶段：

- 阶段 5：全文抓取与文本产物准备
- 阶段 6：引用定位与情感分析

目标是把二者的边界、默认主路径、兼容增强路径和验证口径写清楚，避免 README、主 MVP 计划和当前代码继续漂移。

## 范围

- 包含：
  - 定义阶段 5 与阶段 6 的共享对象与状态边界
  - 冻结 `PDF-first` 主路径与 `GROBID` / `TeX` 兼容路径的关系
  - 明确全文入口、引用定位、上下文抽取与情感分类的步骤
  - 规划“无法判断”路径与证据暴露方式
  - 规划阶段 5 / 阶段 6 的验证脚本与样本
  - 规划与阶段 2 / 阶段 7 的输入输出衔接
- 不包含：
  - 自训练监督模型
  - 人工标注工作台
  - 最终报告排版
...
- 阶段 5 只负责：拿到全文材料、解析、落盘、保留后续可复用产物
- 阶段 6 只负责：基于阶段 5 产物恢复目标引用位置、抽取上下文、输出情感标签
- 默认主路径：
  - 阶段 5：`PDF-first`
  - 阶段 6：`PDF -> GROBID -> context`
- 兼容增强路径：
  - 阶段 5：HTML / LaTeX 仍可解析，但不是默认优先产物
  - 阶段 6：GROBID 不可用时退回普通文本窗口；直接 TeX bibliography / cite-key 仅作为兼容增强链路
- 验证口径必须区分：
  - 本地夹具验证
  - 真实样本验证
  - 在线依赖验证（如 GROBID / arXiv）

## 阶段目标拆解

### 目标 A：先冻结阶段边界

在当前仓库里，阶段 5 和阶段 6 已经有真实代码与验证脚本，因此这份计划首先不是“从零设计”，而是把已经存在的实现收口成一致契约。
...
阶段 5 / 阶段 6 不能把“找全文”“找引用句子”“做情感分类”混成一层抽象。
...
阶段 6 / 当前分支上的引用情感分析模块建议采用 `LangGraph` 驱动的固定工作流，而不是自由式 ReAct。
...
2. 阶段 5 获取全文、保存 `raw artifact + parsed txt`
3. 阶段 6 优先基于 PDF 走 GROBID 路径恢复目标引用
4. GROBID 不可用时退回普通文本窗口路径
5. 对直接 TeX 来源保留 bibliography / cite-key 兼容路径
...
- [x] 新建阶段 5 细化执行计划
- [x] 冻结当前阶段 5 / 阶段 6 边界
- [x] 定义 `CitationContext` / `SentimentSummary`
- [x] 明确全文入口与 `PDF-first` 获取策略
- [x] 设计引用定位与上下文抽取策略
- [x] 设计情感标签规则
- [x] 规划 `packages/sentiment/` 模块边界
- [x] 规划 `scripts/test_agent/stage5.py` / `stage6.py` 验证入口
- [x] 将阶段 5 计划与父计划建立引用关系
- [x] 将当前实现状态同步回主 MVP 计划和聚合验证入口
...
- 2026-04-29：确认当前正式阶段边界为“阶段 5 = 全文抓取与文本产物准备，阶段 6 = 引用定位与情感分析”；这份文件继续覆盖两个相邻阶段，但不再把两者误写成同一个阶段目标。
