...
当前真实前端表面是 `packages/reporting/service.py` 生成的静态 HTML 报告页面，以及 `packages/reporting/pdf_renderer.py` 生成的独立 PDF 报告版式。

## 当前前端边界

- 交付形式：HTML 报告页面 + JSON 数据导出 + PDF 报告
...
- 页面导航：overview / metrics / summary / charts / scholars / findings / attention / contexts
- 指标网格：关键指标卡 + trend / country / institution / scholar / sentiment 图表区
- 报告增强区：分析摘要、重要学者表格、代表性引用语境
