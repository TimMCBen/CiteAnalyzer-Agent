...
- `ci.yml`：仓库级检查，覆盖 docs、repo hygiene、Markdown 和 shell 脚本校验；Markdown lint 默认排除导入型外部资料目录，避免 vendor / coursework 文档格式污染仓库门禁。
- `ci.yml` 当前会在项目级验证中执行 Stage 7 fixture 验证；若仓库 Secrets 提供非空 `API_KEY`，则额外执行 Stage 7 真实 LLM smoke，workflow 固定传入 `MODEL=gpt-5.4`。
