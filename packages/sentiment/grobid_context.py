"""Extract target-citation contexts from GROBID TEI output."""
...
def locate_reference_context_from_grobid_pdf(
    ...
    """Run GROBID on a PDF and locate the target-paper citation context."""
...
def locate_reference_context_from_grobid_tei(tei_path: Path, target_paper: TargetPaper) -> ReferenceMatch:
    """Locate a target-paper bibliography entry and its body citation in TEI XML."""
...
def find_target_bibl_struct(root: ET.Element, target_paper: TargetPaper) -> Optional[ET.Element]:
    """Find the bibliography entry that matches the target DOI or exact title."""
...
def extract_contexts_for_bibl_id(root: ET.Element, bibl_id: str) -> list[str]:
    """Return body paragraphs that cite a matched GROBID bibliography identifier."""
...
def normalize_ws(text: str) -> str:
    """Collapse whitespace in TEI-derived text snippets."""
    return " ".join(text.split())


def serialize_paragraph_with_target_markup(paragraph: ET.Element, target_ref: str) -> str:
    """Render a TEI paragraph while marking the target citation for the LLM."""
    parts: list[str] = []
