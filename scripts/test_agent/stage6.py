...
def load_stage2_sample(sample_path: Path) -> tuple[TargetPaper, list[CitingPaper]]:
    ...
    target_paper = TargetPaper(
        ...
        source_ids={"doi": payload["target_doi"], "crossref": payload["target_doi"]},
...
def maybe_run_real_citing5_smoke(sample_path: Path = DEFAULT_SAMPLE_PATH) -> None:
    ...
    save_dir = REPO_ROOT / "downloaded-papers" / "stage6-live"
    doc = fetch_fulltext_document(paper, search_arxiv_fallback=True, save_dir=save_dir)
    assert doc is not None, "stage6 real citing-5 smoke could not fetch fulltext"
    ...
    result = analyze_citation_sentiments(
        ...
        fulltext_documents={
            "citing-5": FullTextDocument(
                citing_paper_id=doc.citing_paper_id,
                text=doc.text,
                source_type=doc.source_type,
                source_label=doc.source_label,
                local_path=doc.local_path,
                raw_path=doc.raw_path,
                extracted_dir=doc.extracted_dir,
            )
        },
        allow_network=False,
        search_arxiv_fallback=False,
